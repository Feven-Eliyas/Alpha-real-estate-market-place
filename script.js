...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (signInNavBtn) signInNavBtn.addEventListener("click", openAuth);
    if (authClose) authClose.addEventListener("click", closeAuth);
    if (authModal) {
        authModal.addEventListener("click", function (e) {
            if (e.target === authModal) closeAuth();
        });
        const actualSignIn = Array.from(authModal.querySelectorAll("button")).find(function (button) {
            return button.textContent.trim().toLowerCase() === "sign in";
        });
        if (actualSignIn) {
            actualSignIn.addEventListener("click", function () {
                const email = authModal.querySelector('input[type="email"]');
                const password = authModal.querySelector('input[type="password"]');
                if (email && email.value.trim() && password && password.value.trim()) {
                    setTimeout(closeAuth, 0);
                }
            });
        }
    }
    ...
    if (signUpNavBtn) {
        signUpNavBtn.addEventListener("click", function () {
            if (!authModal) return;
            const createLink = Array.from(authModal.querySelectorAll("b")).find(function (item) {
                return item.textContent.toLowerCase().includes("create an account");
            });
    ...
    if (settingsNavBtn) settingsNavBtn.addEventListener("click", openSettings);
    ...
    if (saveSettingsBtn) {
        saveSettingsBtn.addEventListener("click", function () {
            settingsModal.querySelectorAll("input, select").forEach(function (field) {
                const key = "alphaSetting_" + (field.id || field.name || field.placeholder || "field");
    ...
    /* Seller listing details are management-only: no buyer viewing request. */
    if (document.getElementById("seller")) {
        document.addEventListener("click", function () {
            const detailBody = document.getElementById("detailBody");
            if (!detailBody) return;
            detailBody.querySelectorAll("button").forEach(function (button) {
                if (button.textContent.trim().toLowerCase().includes("request a view")) {
                    button.remove();
                }
            });
        }, true);
    ...
    /* Restore saved heart state and provide an actual Favorites view. */
    function savedFavorites() {
        try { return JSON.parse(localStorage.getItem("alphaFavorites") || "[]"); }
        catch (e) { return []; }
    }

    function syncHeartState() {
        const names = savedFavorites();
        document.querySelectorAll("#cards .card").forEach(function (card) {
            const title = card.querySelector("h3");
            const heart = card.querySelector(".heart");
            if (title && heart) heart.textContent = names.includes(title.textContent.trim()) ? "♥" : "♡";
        });
    }
    syncHeartState();
    ...
    function renderFavorites() {
        ...
        const matches = cards.filter(function (card) {
            const title = card.querySelector("h3");
            return title && names.includes(title.textContent.trim());
        });
        if (!matches.length) {
            favoritesList.innerHTML = '<p class="favorite-empty">No favorite properties yet. Tap ♡ on a house to add it here.</p>';
            return;
        }
        matches.forEach(function (card) {
            const copy = card.cloneNode(true);
            const heart = copy.querySelector(".heart");
            if (heart) heart.remove();
            favoritesList.appendChild(copy);
        });
    }
    ...
    if (showFavoritesBtn) showFavoritesBtn.addEventListener("click", openFavorites);
    if (favoritesClose) favoritesClose.addEventListener("click", closeFavorites);
    if (favoritesModal) favoritesModal.addEventListener("click", function (e) { if (e.target === favoritesModal) closeFavorites(); });

    document.querySelectorAll("#cards .heart").forEach(function (heart) {
        heart.addEventListener("click", function () { setTimeout(syncHeartState, 0); });
    });
